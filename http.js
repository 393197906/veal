...
var methods$1 = function methods(next) {
  ...
  var methodNames = ['get', 'post', 'del', 'put'];
  return methods.reduce(function (container, key, index) {
    var _extends3;

    if (key === 'GET') {
      var _extends2;

      return _extends({}, container, (_extends2 = {}, _extends2[methodNames[index]] = function () {
        ...
        var url = args[0],
            params = args[1];
        var strParams = urlEncode(params).slice(1);
    ...
    return _extends({}, container, (_extends3 = {}, _extends3[methodNames[index]] = function () {
...
var filter = function filter(errorHandler, chain) {
  ...
  return function (next) {
    return function () {
      return next.apply(void 0, arguments).then(chain) // return data
      .then(function (data) {
        return {
          data: data
        };
...
var headers = function headers(_headers) {
  ...
  return function (next) {
    return function () {
      ...
      var url = argv[0],
          _argv$ = argv[1],
          params = _argv$ === void 0 ? {} : _argv$;
      var handleHeaders = Object.keys(_headers).reduce(function (conatiner, key) {
        var _extends4;

        var item = typeof _headers[key] === "function" ? _headers[key]() : _headers[key];
        return _extends({}, conatiner, (_extends4 = {}, _extends4[key] = item, _extends4));
      }, {});

      if (params.headers) {
        params.headers = _extends({}, params.headers, handleHeaders);
      } else {
...
var log = function log(next) {
  return function () {
    ...
    var url = params[0],
        argv = params[1];
    var start = Date.now();
    return next.apply(void 0, params).then(function (data) {
      var img = data.err ? 'https://tu.jiuwa.net/pic/20170910/1505053332939917.jpg' : 'http://pic.9ht.com/up/2018-2/15181644163000303.jpg';
      console.group('API接口调用');
      console.log('%c  ', "background:url(\"" + img + "\");padding:45px;background-size:100%; background-repeat:no-repeat;");
      console.log("%c URL \u8DEF\u5F84:", 'color:#CC33CC', url); // console.log(' ')
...
var createFetch = function createFetch(hof) {
...
var applyMiddleware = function applyMiddleware() {
  ...
  var filterMiddlewares = middlewares.filter(isFunction);
  return function (fetch) {
    return compose.apply(void 0, filterMiddlewares)(fetch);
  };
};
